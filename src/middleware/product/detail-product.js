...
const detailProduct = (req, res, next) => {
  ...
  const sql = `SELECT barang.id, barang.sku as "SKU", barang.deskripsi as "Deskripsi", barang.foto as "Foto", brand.nama_brand as "Brand", varian.jenis_barang as "Kategori Product", barang.varian FROM varian INNER JOIN barang ON varian.id_barang = barang.id INNER JOIN brand ON barang.id_brand = brand.id WHERE barang.id = ?`;
